...
public class OVRPlayerController : MonoBehaviour
{
	...
	public virtual void UpdateMovement()
	{
		...
		/*
		// * rewrite this so that it can take controller input, not just keyboard
	...
	*/
		...
		float verticalStickInput = -Input.GetAxis ("Vertical");
		if (verticalStickInput > .1f)
			moveForward = true;
		else if (verticalStickInput < -.1f)
			moveBack = true;
		else 
		{
			moveForward = false;
		...
		float horizontalStickInput = -Input.GetAxis ("Horizontal");
		if (horizontalStickInput > .1f)
			moveLeft = true;
		else if (horizontalStickInput < -.1f)
			moveRight = true;
		else 
		{
			moveLeft = false;
